#include <cmath>
#include "characters_db_utils.hpp"

int CharactersDBUtils::encode_color(const Color& color)
{
...
Color CharactersDBUtils::decode_color(int encodedColor)
{
...
int CharactersDBUtils::convert_double_color_to_int(double color)
{
    return std::max(0, std::min(255, static_cast<int>(std::floor(color * 256.0))));
}

double CharactersDBUtils::convert_int_color_to_double(int color)
{
    return color / 255.0;
}
